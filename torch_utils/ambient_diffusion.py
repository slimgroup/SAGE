...
def get_random_column_mask(image_shape, survival_probability, same_for_all_batch=False, device='cuda', seed=None):
    """Creates a mask with random columns being masked.
        Args:
            image_shape: (batch_size, num_channels, height, width)
            survival_probability: probability of a column being unmasked
    ...
    if same_for_all_batch:
        corruption_mask = np.random.binomial(1, survival_probability, size=(1, 1, width)).astype(np.float32)
        corruption_mask = torch.tensor(corruption_mask, device=device, dtype=torch.float32).repeat([batch_size, num_channels, height, 1])
    else:
        corruption_mask = np.random.binomial(1, survival_probability, size=(batch_size, 1, width)).astype(np.float32)
        corruption_mask = torch.tensor(corruption_mask, device=device, dtype=torch.float32).repeat([1, num_channels, height, 1])
...
def get_well_mask_from_existing(image_shape, num_columns_to_survive, existing_mask, same_for_all_batch=False, device='cuda'):
    """Creates a mask with columns being masked from an existing mask.
        Args:
            image_shape: (batch_size, num_channels, height, width)
            num_columns_to_survive: number of columns to be unmasked
            existing_mask: existing mask to choose columns from
            same_for_all_batch: if True, the same mask is applied to all images in the batch
            device: device to use for the mask
        Returns:
            mask: (batch_size, num_channels, height, width)
    ...
    # Get the indices of the surviving columns in the existing mask
    survive_columns = np.where(existing_mask[0, 0, :] == 1)[0]

    # Randomly choose columns to survive from the surviving columns
    survive_columns = np.random.choice(survive_columns, num_columns_to_survive, replace=False)

    # Create a new mask with only the surviving columns
    corruption_mask = np.zeros(width, dtype=np.float32)
    corruption_mask[survive_columns] = 1
