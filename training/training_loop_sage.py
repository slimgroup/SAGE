...
def training_loop(
    ...
    images, cond, corruption_matrix, hat_corruption_matrix = dataset_iter
    print("Condition has this number of channels:")
    print(cond.shape[1])
    print(corruption_matrix.shape[1])
    print(hat_corruption_matrix.shape[1])
    num_chan_total = images.shape[1] + cond.shape[1] + hat_corruption_matrix.shape[1]
    ...
    while True:
        ...
        for round_idx in range(num_accumulation_rounds):
            with misc.ddp_sync(ddp, (round_idx == num_accumulation_rounds - 1)):
                ...
                corruption_matrix = corruption_matrix.to(device).to(torch.float32)
                hat_corruption_matrix = hat_corruption_matrix.to(device).to(torch.float32)

                train_loss, val_loss = loss_fn(net=ddp, images=images, cond=cond,corruption_matrix = corruption_matrix, \
                                        hat_corruption_matrix = hat_corruption_matrix, augment_pipe=augment_pipe)

                training_stats.report('Loss/loss_train', train_loss)
                training_stats.report('Loss/loss_validation', val_loss)

                scalar_loss = val_loss.sum().mul(loss_scaling / batch_gpu_total)
        ...
        dist.print0("Current n image")
        dist.print0(cur_nimg)
